# ---------------------------
# HausMate Match (Streamlit)
# - Multi-step survey (13 columnas Excel)
# - Branding
# - Madrid neighborhoods map (auto-download GeoJSON)
# - Click map -> detect neighborhood (point in polygon) -> multi-select
# - Scoring visible
# - Anti-duplicados (upsert por teléfono E.164)
# - Export Excel (.xlsx) con columnas exactas
# ---------------------------
...
# ---------------------------
# Remote GeoJSON (no downloads needed on laptop)
# NOTE: This is a public GeoJSON for Madrid "neighborhoods" (commonly used dataset).
# If you later want the official 131 barrios from Ayuntamiento, we can swap the URL.
# ---------------------------
...
def calc_score(data: dict) -> tuple[int, dict]:
    """
    Baseline scoring 0-100 (ajustable).
    """
    score = 0
    breakdown = {}
    ...
    # 3) Fechas (0-20): inicio cercano mejor
    today = date.today()
    if data.get("inicio"):
...
@st.cache_data
def load_barrios_geojson(path: str = "data/madrid_barrios.geojson") -> gpd.GeoDataFrame:
    ...
    # Normalize name column to 'barrio_name'
    # click-that-hood typically uses "name"
...
def barrio_from_click(gdf: gpd.GeoDataFrame, lat: float, lon: float):
    """
    Returns barrio_name that contains clicked point.
    """
    p = Point(lon, lat)

    # Try spatial index for speed; fallback if not available
    try:
        sindex = gdf.sindex
...
# NOTE: Streamlit Cloud storage can reset. Later we can move to Supabase.
# ---------------------------
...
def header():
    ...
    with col1:
        # Optional logo (if you later upload it to assets/hausmate_logo.png)
        try:
...
def render_map(gdf: gpd.GeoDataFrame):
...
def main():
    ...
    # Load barrios GeoJSON (auto-download)
    try:
    ...
    # STEP 1: Preferencias
        ...
        st.session_state.genero = st.selectbox("Género", ["", "Hombre", "Mujer", "Otro", "Prefiero no decir"],
                                              index=["", "Hombre", "Mujer", "Otro", "Prefiero no decir"].index(st.session_state.genero)
                                              if st.session_state.genero in ["", "Hombre", "Mujer", "Otro", "Prefiero no decir"] else 0)
        st.session_state.pref_genero = st.selectbox("¿Con quién quieres vivir?", ["", "Solo hombres", "Solo mujeres", "Mixto", "Me da igual"],
                                                   index=["", "Solo hombres", "Solo mujeres", "Mixto", "Me da igual"].index(st.session_state.pref_genero)
                                                   if st.session_state.pref_genero in ["", "Solo hombres", "Solo mujeres", "Mixto", "Me da igual"] else 0)
        nav_buttons()
    ...
    # STEP 3: Zonas (Mapa)
        ...
        st.caption("Haz click en el mapa: detectamos el barrio automáticamente y lo agregamos a tu selección.")
        if barrios_gdf is None:
            st.warning("Mapa no disponible aún (falló la carga). Intenta recargar la página.")
        else:
    ...
    # STEP 4: Budget
        ...
        st.session_state.budget = st.number_input("Budget (€)", min_value=200, max_value=20000,
                                                  value=int(st.session_state.budget), step=50)
        nav_buttons()
    ...
    # STEP 8: Final
        ...
        if not phone_e164:
            ...
            # Export (solo tus 13 columnas y orden exacto)
            df_store = load_store()
            export_cols = [
